...
for epoch in range(num_epochs):
    ...
    summary_string = 'Epoch{0}/{1}: d_real_loss-{2:.2f},d_fake_loss-{3:.2f},d_total_loss-{4:.2f},G_score-{5:.2f},G_total_loss-{6:.2f},W_dist-{7:.2f},S_losses-{8:.2f}' \
        .format(epoch + 1, num_epochs, np.mean(d_real_losses), np.mean(d_fake_losses), np.mean(D_losses),
                np.mean(G_score), np.mean(G_losses), np.mean(W_dist), np.mean(S_losses))
