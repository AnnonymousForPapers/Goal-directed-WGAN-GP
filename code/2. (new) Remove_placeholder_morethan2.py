# -*- coding: utf-8 -*-
"""
Created on Sun Jan 14 22:56:17 2024
...
for j in range(len(ep)):
    ...
    Bladder = pd.read_csv(data_file + 'Data/' + method + '/' + ep[j] + '/deepimmuno-GANRL-bladder-epoch' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) + '.txt',sep='\t')
    ...
    import os
    outdir = data_file + 'Data/' + method + '/' + ep[j] + '/'
    print("outdir is {}".format(outdir))
    outname = 'deepimmuno-GANRL-bladder-epoch' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) + '_rmv.txt'
...
# Database: http://biopharm.zju.edu.cn/tsnadb/
#Bladder = pd.read_csv('D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/deepimmuno-GANRL-bladder-' + ep + '.txt',sep='\t')
Bladder = pd.read_csv(data_file + 'Data/' + method + '/' + ep + '/deepimmuno-GANRL-bladder-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt',sep='\t')
# https://pandas.pydata.org/docs/reference/api/pandas.Series.str.count.html
Bladder_count = Bladder['peptide'].str.count('-')
# https://stackoverflow.com/questions/17071871/how-do-i-select-rows-from-a-dataframe-based-on-column-values
Bladder_rmv = Bladder.loc[Bladder_count<2]
