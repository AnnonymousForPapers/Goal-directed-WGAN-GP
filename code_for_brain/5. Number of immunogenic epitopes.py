...
for j in range(len(ep)):
    #%%
    indir = data_file + 'result_for_brain/' + method + '/' + ep[j] + '/'
    print("indir is {}".format(indir))
    data_path = 'deepimmuno-GANRL-brain_cancer-scored_epoch' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) + '.txt'
...
ax.legend(bbox_to_anchor=(0.0, 0.9), loc='upper left', fontsize=14)
ax.set_ylim(0, 1100)
