...
for j in range(len(epoch)):
    ...
    data_path = data_file + 'result_for_brain/' + method + '/' + ep + '/deepimmuno-GANRL-brain-epoch' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) + '_rmv.txt'
    ...
    with open(data_file + 'result_for_brain/' + method + '/' + ep + '/epoch[j]' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) +'log_rmv_0.txt','w') as f:
        for item1 in seq:
            total = []
            for item2 in raw:
                total.append((item2,similar(item1,item2)))
...
ep = 'epoch' + str(epoch)
# data_path = 'D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/deepimmuno-GANRL-bladder-' + ep + '_rmv.txt'
data_path = data_file + 'result_for_brain/' + method + '/' + ep + '/deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '_rmv.txt'
generate = pd.read_csv(data_path,sep='\t')
...
count = 0
#with open('D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/log_rmv_0.txt','w') as f:
with open(data_file + 'result_for_brain/' + method + '/' + ep + '/epoch' + str(num_epochs) + '-batch' + str(batch_size) +'log_rmv_0.txt','w') as f:
    for item1 in seq:
        total = []
        for item2 in raw:
            total.append((item2,similar(item1,item2)))
