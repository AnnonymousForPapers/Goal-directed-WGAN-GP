# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 22:27:12 2024
...
input_dir1 = data_file + 'result_for_brain/' + method + '/' + 'epoch' + str(num_epochs)
score1 = np.load(input_dir1+'/G_max_score.npy')
temp = 0
acu_score1 = []
...
# 1. GAN+CNN (no label)
method = 'WGAN-GP'
input_dir2 = data_file + 'result_for_brain/' + method + '/' + 'epoch' + str(num_epochs)
score2 = np.load(input_dir2+'/G_max_score.npy')
temp = 0
acu_score2 = []
for i in range(0, num_epochs, 1): # range(start, stop, step)
    temp = temp + score2[i]
...
plt.ylabel('Maximum immunogenicity score',fontsize = 20, weight='bold')
