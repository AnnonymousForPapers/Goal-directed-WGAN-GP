...
output_dir = data_file + 'result_for_brain/Goal-directed_WGAN-GP/epoch' + str(num_epochs)
...
for epoch in range(num_epochs):
    ...
    S_losses = []
    G_losses, D_losses, W_dist, G_score, G_max_score, G_dvalue = [], [], [], [], [], []
    ...
    for mini_batch in real_dataloader:
        ...
        s_error_total = Score_loss(s_fake_pred,fake_scores) + Score_loss(s_real_pred,real_scores) 
    ...
    summary_string = 'Epoch{0}/{1}: d_real_loss-{2:.2f},d_fake_loss-{3:.2f},d_total_loss-{4:.2f},G_score-{5:.2f},G_total_loss-{6:.2f},W_dist-{7:.2f},S_losses-{8:.2f},G_max_score-{9:.2f}' \
        .format(epoch + 1, num_epochs, np.mean(d_real_losses), np.mean(d_fake_losses), np.mean(D_losses),
                np.mean(G_score), np.mean(G_losses), np.mean(W_dist), np.mean(S_losses), np.mean(G_max_score))
